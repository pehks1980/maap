...
class MulApp:
    ...
    def finish(self,f_time):
        ...
        if self.ans_num >1:
            ...
            for i in sub_li1:
                if i[2]==1:
                    reply.append(f' {i[0]} X {i[1]} (={i[0] * i[1]}) занял {i[3]} секунд (порог {self.favor_thresold_time}) сек')
                if i[2]==2:
                    reply.append(f' {i[0]} + {i[1]} (={i[0] + i[1]}) занял {i[3]} секунд (порог {self.favor_thresold_time}) сек')
                if i[2]==3:
                    reply.append(f' {i[0]} - {i[1]} (={i[0] - i[1]}) занял {i[3]} секунд (порог {self.favor_thresold_time}) сек')
